...
GOPHERNOTES_SRC = github.com/gopherdata/gophernotes
...
build-gophernotes: ### Build gophernotes
	rm -rf build
	rm -rf release
	mkdir -p build/linux/amd64 && GOOS=linux GOARCH=amd64 go build -o build/linux/amd64/gophernotes $(GOPHERNOTES_SRC)
	mkdir -p build/darwin/amd64 && GOOS=darwin GOARCH=amd64 go build -o build/darwin/amd64/gophernotes $(GOPHERNOTES_SRC)
	mkdir -p release/
	tar -cvzf release/gophernotes-linux-amd64.tar.gz -C build/linux/amd64 gophernotes
	tar -cvzf release/gophernotes-darwin-amd64.tar.gz -C build/darwin/amd64 gophernotes
	$(SHA512SUM) release/gophernotes-linux-amd64.tar.gz >release/gophernotes-linux-amd64.tar.gz.sha256sum
	$(SHA512SUM) release/gophernotes-darwin-amd64.tar.gz >release/gophernotes-darwin-amd64.tar.gz.sha256sum